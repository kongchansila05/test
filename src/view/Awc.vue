...
export default {
    ...
    methods: {
        //=====getBennerAwc=========
        getBennerAwc(){
            axios.get("https://admin.kh.2m-sport.com/api/bennerawc").then((res)=>{
                this.BennerAwc = res.data.data;
        ...
        //=====category=========
        GetAwc(){
        axios.get("https://admin.kh.2m-sport.com/api/awc").then((res)=>{
            this.Awc = res.data.data;
        }).catch(function (error) {
            console.error(error);
        });
        },
...
<template>
    ...
    <div v-for="(value, key, index) in BennerAwc" :key="index">
        ...
        <vue-load-image v-if="lang == 'kh'">
            ...
            <v-lazy-image :src="'https://admin.kh.2m-sport.com/upload/benner/'+value.photo_khmer" :src-placeholder="'https://admin.kh.2m-sport.com/upload/'+value.photo_khmer" style="width: 100%;height: 100%; " />
            </template>
            <template v-slot:error>Benner</template>
        </vue-load-image>
        <vue-load-image v-else>
            ...
            <template v-slot:preloader> 
            <v-lazy-image :src="'https://admin.kh.2m-sport.com/upload/benner/'+value.photo" :src-placeholder="'https://admin.kh.2m-sport.com/upload/'+value.photo" style="width: 100%;height: 100%; " />
            </template>
            <template v-slot:error>Benner</template>
        </vue-load-image>
    </div>
...
<div class="main-content" >
    <div class="container ">
        <div class="grid-footer game-list" data-aos="zoom-in">
            <div v-for="(value, key, index) in Awc" :key="index">
