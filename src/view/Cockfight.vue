...
export default {
  ...
  mounted(){
        this.GetCockfight();
        this.getBennerCockfight();
    },
    methods: {
        //=====getBennerCockfight=========
        getBennerCockfight(){
            axios.get("https://admin.kh.2m-sport.com/api/bennercockfight").then((res)=>{
                this.BennerCockfight = res.data.data;
        ...
        //=====category=========
        GetCockfight(){
        axios.get("https://admin.kh.2m-sport.com/api/cockfight").then((res)=>{
            this.Cockfight = res.data.data;
        }).catch(function (error) {
            console.error(error);
        });
        },
...
<template>
    <div class="video">
    <video src="/benner/cockfight.mp4" title="2m-sport" type="video/mp4" autoplay="autoplay" loop="loop" muted="muted" playsinline="" class="cursor-pointer" style="width:100%;"></video>
  </div>
    <!-- <div v-for="(value, key, index) in BennerCockfight" :key="index">
...
<div class="main-content" >
    <div class="container">
        <div class="grid-footer game-list stage" data-aos="zoom-in">
            <div v-for="(value, key, index) in Cockfight" :key="index">
                <div v-if="value.photo" class="box-footer game-single-list box bounce-1" data-aos="zoom-in" style="border-radius: 5px;overflow: hidden;">
                    <vue-load-image >
                        ...
                        <template v-slot:preloader> 
                            <img src="/blank/cockfight.png" alt="" style="width: 100%;height: 100%; ">
                        </template>
                        <template v-slot:error>Image load fails</template>
                    </vue-load-image>
